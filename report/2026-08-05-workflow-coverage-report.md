...
- 当前部署 `7a7781067` / `7a778106` 健康，包含 managed Codex NyxID `X-API-Key`、actor-owned provisioning 和 binder-attested 只读窄放行；`0c4ff023` 的 17-case 全量回归仍作为其余案例基线。Fresh schedule 复验从 HTTP 200 confirmation、HTTP 202 typed receipt、binding/provisioning committed success，一直覆盖到 schedule 回读与单次 `run-now`。触发后 `fireCount=1`、`failureCount=0`，案例 15 workflow 11/11 committed `completed`、`stateVersion=73`。`f7f543c5` 上的 `NyxIdOperationAuthorityContractUnavailable` 仅保留为历史回归背景。
...
| 预算监控 | 5 | 04、08、15 + 源 P2 no-send | 六路 Base、预算差异、阈值、周/月摘要、消息、Durable schedule | 源 no-send 已 8/8 完成；公开案例 15 schedule 在 `7a778106` 上已创建、回读并单次触发成功；源 send 与源 durable schedule 未运行 | 公开主链与排程覆盖 / 源副作用边界未运行 |
...
本节同时记录历史基线、已部署工程修复和 2026-08-06 SGT 的 fresh NyxID 端到端复验。历史 HTTP 502 与 `f7f543c5` authority failure 仍作为回归背景保留，当前生产事实以 `7a7781067` / `7a778106` 为准；新旧证据不会相互覆盖。
...
| 源码后续修复 | `7a7781067` 已进入 `origin/feature/integrate` 并随镜像 `7a778106` 部署：完整 Durable proof、binder grant 与 service catalog 校验通过后，仅 binder-attested `READ_ONLY` GET/HEAD/OPTIONS 跳过独立 operation-authority preview；NyxID runtime proxy 仍逐次校验当前权限，POST、WRITE 与 DESTRUCTIVE 继续 fail-closed | 精确适用于案例 15 的 6 个 GET；生产已验证 |
| 查询与回执 | member read model 可见；binding committed `succeeded`（state 7），provisioning 首次 attempt committed `succeeded`（state 9）；schedule/operation ID 均非空。schedule 回读为 enabled，Cron `0 9 * * 1`、时区 `Asia/Singapore` | Durable 创建与读取通过 |
| 真实触发 | `run-now` 只提交一次；随后 `fireCount=1`、`failureCount=0`。workflow run 11/11 committed `completed`（state 73），最终 artifact 命中六路 live Base、周度 2340/2400、月度 9360/9600、over=1、watch=1 | Durable schedule 端到端生产通过 |
| 测试与门禁 | 23/23、1730/1730、152/152、Mainnet DI composition 1/1、Studio DI/executor 11/11、Capabilities 642/642 通过；第二轮全量 solution test、solution split、架构/边界门禁与 `slow_test_guards.sh` 均通过。首轮仅有一个 Orleans retry 测试 20 秒超时，隔离复跑与第二轮全量均通过。测试工作树基于 `7a7781067`，另含 4 个未提交的 fixture/boot/admitted-terminal 修复文件；这些本地变化不计为已部署内容 | 本地验证通过；提交边界已标明 |
...
| 15 | 六路 Base、预算周报/月报、schedule | 通过 | `7a778106` 上 binding/provisioning succeeded，schedule 可读取；单次 run-now 后 fire=1/failure=0，workflow 11/11 committed `completed`，`stateVersion=73` | 核心业务与公开 Durable schedule 端到端通过 |
