...
ALLOWED_STATUSES = %w[passed blocked failed pending-execution not-configured].freeze
FORBIDDEN_KEYS = %w[
...
fail_validation("案例 27 public catalog 数量必须为 25") unless
  cases.dig("27", "target", "expected_skill_count") == 25 &&
  cases.dig("27", "required_evidence", "public_skill_count") == 25
...
fail_validation("案例 42 必须先隔离旧定义中的硬编码集成语义") unless
  cases.dig("42", "required_evidence", "legacy_integration_semantics_isolated") == true &&
  cases.dig("42", "required_evidence", "hard_coded_targets_absent") == true
...
workflow_case_ids = Dir[File.join(ROOT, "workflows", "*.workflow.yaml")]
  ...
  .sort
fail_validation("25 个 direct workflow 未全部注册严格 contract") unless
  workflow_case_ids == (1..25).map { |number| format("%02d", number) } &&
  workflow_case_ids == RuntimeContracts::CONTRACTS.keys
...
fail_validation("生产部署未证明包含目标提交") unless
  evidence.dig("productionDeployment", "requiredSourceCommitPresent") == true &&
  evidence.dig("productionDeployment", "readyReplicas") == "1/1" &&
  evidence.dig("productionDeployment", "commit") == SOURCE_COMMIT
...
results.each do |item|
  ...
  when "passed"
    ...
    fail_validation("案例 #{case_id} passed 但未满足 required evidence") unless
...
fail_validation("案例 23 未保存拒绝缺少 typed code 的真实失败") unless
...
fail_validation("案例 37 未保存 workflow_call definition resolution 超时") unless
  result_by_case.dig("37", "stableErrorCode") == "WORKFLOW_CALL_DEFINITION_RESOLUTION_TIMEOUT" &&
  result_by_case.dig("37", "actualEvidence", "runHash") == "4327dd64a0b0" &&
  result_by_case.dig("37", "actualEvidence", "definitionResolutionTimeoutMs") == 30_000 &&
  result_by_case.dig("37", "actualEvidence", "childWorkflowStarted") == false
fail_validation("案例 38/39 未保存 hard-coded llm_call blocker") unless
  %w[38 39].all? do |case_id|
    result_by_case.dig(case_id, "stableErrorCode") == "DETERMINISTIC_PROBE_UNAVAILABLE" &&
      result_by_case.dig(case_id, "actualEvidence", "workerStepTypeHardCodedToLlmCall") == true &&
      result_by_case.dig(case_id, "actualEvidence", "productionProbeExecuted") == false
  end
fail_validation("案例 40-42 未保存安全配置缺口") unless
  ...
  result_by_case.dig("42", "stableErrorCode") == "LEGACY_INTEGRATION_ISOLATION_NOT_CONFIGURED" &&
  %w[40 41 42].all? { |case_id| result_by_case.dig(case_id, "actualEvidence", "productionRunExecuted") == false }
...
computed_report_evidence = {
  ...
  "readme_workflow_count" => readme.include?("25 个 workflow") ? 25 : 0,
  "report_workflow_count" => report.include?("25/25 个公开 workflow") ? 25 : 0,
  "machine_summary_workflow_count" => 20 + probe_results.length,
...
puts "通过 风险案例=#{results.length} passed=#{counts['passed']} blocked=#{counts['blocked']} " \
     ...
     "not-configured=#{counts['not-configured']}"
